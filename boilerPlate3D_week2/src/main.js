...
import { addPlanet2 } from './addPlanet2'
import { addPlanet3 } from './addPlanet3'
import { addPlanet4 } from './addPlanet4'
import { addPhongMesh } from './addPhongMesh'
import { addLambertMesh } from './addLambertMesh'
...
function init() {
	...
	meshes.planet2 = addPlanet2(new THREE.Color(0x00a0b0), 0.35)
	meshes.planet3 = addPlanet3(new THREE.Color(0x0aab10), 0.5)
	meshes.planet4 = addPlanet4(new THREE.Color(0x00c100), 0.3)
	meshes.phong = addPhongMesh()
	meshes.lambert = addLambertMesh()
...
function animate() {
	...
	//tick function stores our new time every single frame
	...
	meshes.planet2.position.x = Math.sin(-tick) * 3
	meshes.planet2.position.y = Math.cos(-tick) * 3

	meshes.planet3.position.x = Math.sin(-tick * 2) * -1
	meshes.planet3.position.y = Math.cos(-tick * 2) * -1

	meshes.planet4.position.x = Math.sin(tick) * 4
	meshes.planet4.position.y = Math.cos(tick) * 4
